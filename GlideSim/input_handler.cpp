...
void InputHandler::process_input()
{
	...
	if (state == GLFW_PRESS)
		core::camera.move(movement_rate, 0, 0);
	...
	if (state == GLFW_PRESS)
		core::camera.move(0, 0, -movement_rate);
	...
	if (state == GLFW_PRESS)
		core::camera.move(0, movement_rate, 0);
	...
	core::camera.rotate(glm::vec3(0, verticalAngle, horizontalAngle));
